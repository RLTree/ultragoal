...
impl Workspace {
    pub(crate) fn open(path: &Path) -> Result<Self, FitError> {
        let path_metadata =
        ...
        let binding = super::super::root_binding_from_canonical_path(
            &canonical,
            metadata.dev(),
            metadata.ino(),
        );
    ...
    pub(crate) fn root_binding(&self) -> Result<String, FitError> {
        self.verify_root()?;
    ...
    pub(crate) fn read_file(
        ...
        self.verify_root()?;
        if maximum_bytes == 0
            || maximum_bytes > crate::repository_fit::repository_contract::MAX_FILE_BYTES
        {
    ...
    pub(crate) fn revalidate_attachments(
        &self,
