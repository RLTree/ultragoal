...
fn required_claim_removal_and_ceiling_reduction_fail_closed() {
    for mutation in 0..2 {
        let mut rows = fixture::rows();
        if mutation == 0 {
            rows[2]["removed_or_weakened_claim_ids"] = json!(["CL-SOURCE"]);
            rows[2]["derived_removed_or_weakened_claim_ids"] = json!(["CL-SOURCE"]);
        } else {
            rows[2]["after_claim_ceiling"]
                .as_array_mut()
...
fn semantic_policy_tampering_fails_closed() {
    ...
    let mut rows = fixture::rows();
    rows[2]["approval"] = json!({"required": true, "status": "missing"});
    let bytes = fixture::reseal(&mut rows);
    assert!(validate_current(&bytes, fixture::binding()).is_err());
}
...
fn duplicate_current_row_and_backlog_substitution_fail_closed() {
    ...
    mismatch[2]["backlog_updates"][0]["path"] = json!("examples/generated/other.json");
