...
pub(crate) const PRODUCTION_SUPPORT_LIMIT: &str = "source-local routine planning, clean no-op, durable pre-broker reservation, and fail-closed effect refusal on supported macOS hosts only; a root-owned broker authorization cannot be minted here, so executable effects, cancellation, child results, installed behavior, representative effect journeys, readiness, release, and completion remain unavailable";
