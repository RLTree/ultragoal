...
impl Store {
    ...
    pub(crate) fn write_atomic_state(&self, bytes: &[u8]) -> StatePublication {
        ...
        let Ok(temporary_identity) = before_publish else {
        ...
        match (
            ...
            (Ok(identity), Ok(current))
                if identity.device == temporary_identity.device
                    && identity.inode == temporary_identity.inode
                    && current == bytes =>
            {
