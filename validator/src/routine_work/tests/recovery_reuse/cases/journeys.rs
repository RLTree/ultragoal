...
use super::base_support::{fallback_graph, graph};
...
fn clean_dirty_conflict_and_fallback_journeys_preserve_unrelated_work() {
    run_routine_fixture_invocation(
        &[
            "journey-clean",
            "journey-dirty",
            "journey-conflict",
        ...
        |repositories| {
            let [clean, dirty, conflict, fallback] = repositories else {
                panic!("four journey repositories are required")
            };
...
fn first_execution_exact_repeat_reuse_and_mutation_forced_miss_are_causal() {
    run_routine_fixture_invocation(&["journey-repeat-reuse"], |repositories| {
        let [repo] = repositories else {
            panic!("one reuse repository is required")
        };
        ...
        let syntax_receipt =
            capture_receipt(repo, &context, syntax.receipt_json(), "journey-syntax");
