...
fn launch_uses_a_fresh_immutable_owner_only_copy() {
    let bytes = b"#!/bin/sh\nprintf sealed\n";
    let fixture = selected_test_fixture("darwin-copy", bytes);
    let selected = fixture.selected().expect("selected executable");
    let launch = selected.launch_path();
...
fn executor_runs_sealed_copy_inside_contained_scope() {
    ...
    let fixture: SelectedCodexExecutableTestFixture =
