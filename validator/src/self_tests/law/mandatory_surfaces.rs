...
#[test]
fn mandatory_law_receipts_require_real_current_check_evidence() {
    let root = crate::self_tests::boundaries::support::temp_root("mandatory-law-production");
    write_json(
    ...
    let mut current_failures = std::collections::BTreeMap::new();
    ...
    assert!(
        failures
            .iter()
            .any(|failure| failure.contains("mandatory_law_current_check_not_pass")),
        "{failures:?}"
    );

    let mut row_shape_only = law.clone();
    row_shape_only["validator_check_id"] = json!("not-a-real-check");
    let failures =
        crate::audit::mandatory::law::surfaces::receipt_value_failures(&root, &row_shape_only);
    assert!(
        failures
            .iter()
            .any(|failure| failure.contains("mandatory_law_unknown_validator_check")),
        "{failures:?}"
    );
    std::fs::remove_dir_all(root).expect("cleanup mandatory production law");
}
