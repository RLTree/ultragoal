...
fn product_command_reports_observability_receipt_write_failures() {
    ...
    .expect("manifest");
    std::fs::write(
        root.join("validation_artifacts/observability/product-observability-blocker"),
        "blocker",
    )
    .expect("blocker file");
    let err = crate::cli::product::run(
        ...
        &crate::cli::product::ProductCommand {
            ...
            observability_receipt: PathBuf::from(
                "validation_artifacts/observability/product-observability-blocker/receipt.json",
            ),
        },
    )
