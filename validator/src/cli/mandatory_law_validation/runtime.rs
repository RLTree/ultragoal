...
pub(super) fn from_metrics(
    ...
    crate::cli::observe::telemetry::RuntimeTelemetry {
        ...
        cache_mode: unique_text(rows, |row| row.cache_mode, "mandatory_law_no_cache"),
        resource_measurement_status: unique_text(
            rows,
            |row| row.resource_measurement_status,
            "mandatory_law_no_scheduler_metrics",
        ),
        retry_count: 0,
        backoff_ms: 0,
        saturation_status: saturation_status(rows),
        repair_anchor_before: "mandatory_law_validation_command_start".to_string(),
        repair_anchor_after: "mandatory_law_validation_observability_emit".to_string(),
    }
}
...
fn unique_text(
    ...
    match values.len() {
        ...
        _ => "mixed_mandatory_law_scheduler_modes".to_string(),
    }
}
...
fn saturation_status(rows: &[crate::scheduler::Metrics]) -> String {
    ...
    if task_count == 0 {
        ...
        "queued_parallel_mandatory_law_validation".to_string()
    } else {
