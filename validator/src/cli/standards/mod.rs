...
mod stdout {
    ...
    pub(super) fn contract(value: &Value) -> Vec<String> {
        ...
        let metric_query = crate::cli::observe::query::bounded_metric_query_for_operation(text(
            value,
            "operation",
        ));
...
fn write_json(root: &Path, receipt: &Path, value: &Value) -> Result<(), String> {
    let path = crate::output_path::claim_artifact_path(root, receipt, "standards receipt")?;
