...
const DEFAULT_POLICY: &str = "docs/openai-key-policy.json";
const DEFAULT_RECEIPT: &str = "validation_artifacts/openai/config-receipt.json";

#[derive(Debug)]
pub(crate) struct OpenAiCommand {
    receipt: PathBuf,
    policy: PathBuf,
}
...
pub(crate) fn parse(raw: &[String]) -> Result<Option<OpenAiCommand>, String> {
    ...
    match raw {
        [_, action, subject, ..] if action == "config" && subject == "prove" => {
            Ok(Some(OpenAiCommand {
                receipt: opt_path(raw, "--receipt")
                    .unwrap_or_else(|| PathBuf::from(DEFAULT_RECEIPT)),
                policy: opt_path(raw, "--policy").unwrap_or_else(|| PathBuf::from(DEFAULT_POLICY)),
            }))
        }
...
pub(crate) fn run(root: &Path, command: &OpenAiCommand) -> Result<i32, String> {
    let receipt = build_receipt(root, command)?;
...
pub(crate) fn build_receipt(root: &Path, command: &OpenAiCommand) -> Result<Value, String> {
    ...
    let policy = policy::load(root, &command.policy);
    ...
    if policy::contains_secret_shape(&value) {
...
fn print_receipt(receipt: &Path, value: &Value) {
    ...
    if status != "pass" {
        println!(
            "failed_check={LAW_ID} why={} where={} claim_impact=blocks_openai_dependent_claims next_repair=repair_openai_key_policy_or_local_env receipt={}",
            csv(value.get("failures")),
            value
                .get("policy_path")
                .and_then(Value::as_str)
