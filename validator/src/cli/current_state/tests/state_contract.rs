...
pub(crate) fn metadata_snapshot(root: &std::path::Path) -> Vec<(std::path::PathBuf, u64, bool)> {
...
#[test]
pub(crate) fn parser_defaults_to_read_only_and_accepts_explicit_receipt() {
...
#[test]
pub(crate) fn digest_fields_cover_known_receipt_shapes() {
    assert_eq!(
        super::super::receipt_status::digest_field(
            &json!({"target_revision": {"value": "sha256:a"}})
        ),
        Some("sha256:a")
    );
    assert_eq!(
        super::super::receipt_status::digest_field(&json!({"target_digest": "sha256:b"})),
        Some("sha256:b")
    );
    assert_eq!(
        super::super::receipt_status::digest_field(&json!({"candidate_digest": "sha256:c"})),
        Some("sha256:c")
    );
    assert_eq!(super::super::receipt_status::digest_field(&json!({})), None);
}

#[test]
pub(crate) fn receipt_state_reports_current_status_detail_and_missing_digest() {
    let root =
...
#[test]
pub(crate) fn git_status_reports_command_failure_as_dirty() {
...
#[test]
pub(crate) fn first_blocker_prefers_observability_board_then_stale_receipt() {
    let board = json!({
        "status": "unavailable",
...
#[test]
pub(crate) fn current_state_never_reads_static_command_inventory_or_claims_observable() {
    let root = crate::self_tests::boundaries::workspace_fixtures::temp_root(
...
#[test]
pub(crate) fn current_state_default_public_path_is_read_only() {
    let root =
...
#[test]
pub(crate) fn current_state_run_fails_closed_without_candidate_digest() {
    let root = crate::self_tests::boundaries::workspace_fixtures::temp_root(
...
#[test]
pub(crate) fn current_state_run_rejects_external_claim_receipts() {
    let root = crate::self_tests::boundaries::workspace_fixtures::temp_root(
