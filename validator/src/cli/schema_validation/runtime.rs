...
pub(super) fn from_metrics(
    ...
    crate::cli::observe::telemetry::RuntimeTelemetry {
        ...
        cache_mode: unique_text(rows, |row| row.cache_mode, "schema_validation_no_cache"),
        resource_measurement_status: unique_text(
            rows,
            |row| row.resource_measurement_status,
            "schema_validation_no_scheduler_metrics",
        ),
        retry_count: 0,
        backoff_ms: 0,
        saturation_status: saturation_status(rows),
        repair_anchor_before: repair_anchor_before.to_string(),
        repair_anchor_after: "schema_validation_observability_emit".to_string(),
    }
}
...
fn unique_text(
    ...
    match values.len() {
        ...
        _ => "mixed_schema_validation_scheduler_modes".to_string(),
    }
}
...
fn saturation_status(rows: &[crate::scheduler::Metrics]) -> String {
    ...
    if task_count == 0 {
        ...
        "queued_parallel_schema_validation".to_string()
    } else {
