...
pub(crate) const CATALOG_BINDING_UNAVAILABLE: &str =
    "capture unavailable: caller-supplied CommandSpec lacks an accepted typed-catalog binding";
...
/// worktree-relative, and production capture stays fail-closed because no
/// production authority can mint the native read-only catalog binding.
...
impl CommandSpec {
    ...
    /// Construct a test-only, read-only capture request. Production callers
    /// cannot mint this binding.
    #[cfg(test)]
