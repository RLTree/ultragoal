...
pub(super) fn from_metrics(
    ...
    crate::cli::observe::telemetry::RuntimeTelemetry {
        ...
        cache_mode: unique_text(rows, |row| row.cache_mode, "line_caps_no_cache"),
        resource_measurement_status: unique_text(
            rows,
            |row| row.resource_measurement_status,
            "line_caps_no_scheduler_metrics",
        ),
        retry_count: 0,
        backoff_ms: 0,
        saturation_status: saturation_status(rows),
        repair_anchor_before: "line_caps_check_start".to_string(),
        repair_anchor_after: "line_caps_observability_emit".to_string(),
    }
}
...
fn unique_text(
    ...
    match values.len() {
        ...
        _ => "mixed_line_caps_scheduler_modes".to_string(),
    }
}
...
fn saturation_status(rows: &[crate::scheduler::Metrics]) -> String {
    ...
    if task_count == 0 {
        ...
        "queued_parallel_line_cap_checks".to_string()
    } else {
...
mod tests {
    ...
    #[test]
    fn line_cap_runtime_aggregates_optional_resource_metrics() {
        ...
        assert_eq!(runtime.cache_mode, "mixed_line_caps_scheduler_modes");
        assert_eq!(runtime.saturation_status, "queued_parallel_line_cap_checks");
    }

    #[test]
    fn line_cap_runtime_reports_empty_and_missing_worker_states() {
        let empty = super::from_metrics(&[], 1);
        assert_eq!(empty.worker_count, 0);
        assert_eq!(empty.saturation_status, "no_scheduler_tasks_started");
        assert_eq!(empty.cache_mode, "line_caps_no_cache");
