...
pub(super) fn print_summary(value: &Value, archive_digest: &str) {
    println!(
        "ultragoal-archive-build {} proven={} archive_digest={} candidate={} receipt={} run_id={} correlation_id={} claim_impact={} supported_claims={} unsupported_claims={}",
        text(value, "status"),
        if text(value, "status") == "pass" {
            super::archive::CLAIM_ID
        } else {
            "none"
        },
        archive_digest,
...
fn print_failure(value: &Value) {
    ...
    let metric_query =
        crate::cli::observe::query::bounded_metric_query_for_operation(super::archive::OPERATION);
    println!(
