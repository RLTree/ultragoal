...
pub(super) fn verified_cache_row(candidate: &str) -> serde_json::Value {
    ...
    json!({
        ...
        "product_behavior_observed": "cargo fmt --all --check verified cache replay",
        "proof_surface": "speed node receipt with cache key, current input digest, prior result digest, replayed output digest, and invalidation proof",
        "independent_reconciliation_surface": "same-candidate telemetry and verified cache equivalence replay",
...
pub(super) fn executed_row(candidate: &str) -> serde_json::Value {
    json!({
        ...
        "telemetry_reconciliation_status": "pass",
        "claim_name": "source-local speed node timing claim",
        "product_behavior_observed": "cargo fmt --all --check command execution",
        "proof_surface": "speed node receipt with command argv, exit status, result digest, output digest, and telemetry reconciliation",
        "independent_reconciliation_surface": "same-candidate telemetry and performance receipt",
        "claim_impact": "supports_live_loop_node_timing_only_no_readiness_release_completion_update_goal",
        "verified_local_command_argv": ["bash", "-lc", "cargo fmt --all --check"],
        "command_argv": ["bash", "-lc", "cargo fmt --all --check"],
        "verified_local_exit_code": 0,
