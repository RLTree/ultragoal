...
use super::{
    CONFIG_NAME, HostAuthorityConfig, HostAuthorityConfigDefinition, HostError, KEY_BYTES,
    KEY_NAME, LEDGER_NAME, LOCK_NAME, monotonic_nanoseconds, realtime_milliseconds,
    repository_scope_id,
};
...
pub(crate) fn provision_darwin_migration_host_for_test(
    ...
    #[cfg(target_vendor = "apple")]
    {
        ...
        let config = HostAuthorityConfig::issue(HostAuthorityConfigDefinition {
            repository_scope_sha256: scope.clone(),
            candidate_id: inventory.candidate_id().to_owned(),
            product_version: product_version.to_owned(),
            clock_anchor_unix_ms: realtime_milliseconds()?,
            clock_anchor_monotonic_ns: monotonic_nanoseconds()?,
        })?;
