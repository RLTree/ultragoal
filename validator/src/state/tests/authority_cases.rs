...
fn adopted_claims() -> Vec<ClaimSpec> {
    [
...
fn policy(code: &str, dimensions: &[&str]) -> InventoryPolicy {
    InventoryPolicy {
        code: code.to_owned(),
...
fn verify(catalog: &DependencyActionCatalog, candidate_id: Option<&str>) -> Result<(), StateError> {
...
#[test]
fn exact_adopted_claim_policy_issues_and_initializes_only_its_dimensions() {
...
#[test]
fn adopted_claim_authority_rejects_unknown_claims_and_dimensions() {
    let accepted = spec();
...
#[test]
fn inventory_impact_coverage_rejects_missing_extra_duplicate_and_conflicting_rows() {
    assert_eq!(
...
#[test]
fn untrusted_self_digest_and_decoy_shunting_never_initialize_ceilings() {
    let mut forged = spec();
    forged.claims = vec![
        ClaimSpec {
...
#[test]
fn every_permit_binding_and_integrity_mutation_fails_closed() {
...
#[test]
fn permit_substitution_and_concurrent_derivation_preserve_one_binding() {
