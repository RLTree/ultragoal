...
fn derive_root_evidence(
    spec: &EvaluationSpec,
    root: &std::path::Path,
) -> Result<ProductionEvidenceRequest, EvaluationError> {
    ...
    for task in &spec.tasks {
        ...
        let scorer: RootScorerEvidence = serde_json::from_slice(&scorer.read_authenticated()?)
            .map_err(|_| EvaluationError::new("evaluation-production-evidence-invalid"))?;
                ...
                .is_some_and(|row| row != &grader_row)
        {
            return Err(EvaluationError::new(
                "evaluation-production-evidence-invalid",
            ));
        }
        expected_task = Some(task_row);
        expected_grader = Some(grader_row);
    }
    let (task_authority_id, task_principal_id, task_session_id) = expected_task
        .ok_or_else(|| EvaluationError::new("evaluation-production-evidence-invalid"))?;
    let (grader_authority_id, grader_principal_id, grader_session_id) = expected_grader
        .ok_or_else(|| EvaluationError::new("evaluation-production-evidence-invalid"))?;
    ...
    Ok(ProductionEvidenceRequest::from_values(
        task_authority_id,
        task_principal_id,
        task_session_id,
        format!(
