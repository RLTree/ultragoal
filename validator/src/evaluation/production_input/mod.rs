...
impl<'a> ProductionSpecPermit<'a> {
    pub(super) fn issue_from_root(
        ...
        let admission = root_admission::derive_root_admission(spec, root.as_ref())?;
        Self::issue(
            spec,
            root,
            EvaluationAdmissionAuthority::issue(spec, admission)?,
        )
    }
    ...
    fn issue(
        spec: &'a EvaluationSpec,
        root: impl AsRef<std::path::Path>,
        authority: EvaluationAdmissionAuthority,
    ) -> Result<Self, EvaluationError> {
    ...
    pub(crate) fn test_issue(
        ...
        Self::issue(spec, root, EvaluationAdmissionAuthority::test_issue(spec))
    }
}
