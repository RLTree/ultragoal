...
use super::reader_witness_specs::{ACCEPTED_CONTEXT_ARTIFACTS, ArtifactSpec};
...
const GUARD_PATH: &str = "validator/src/inventory/agent_reader_guard_digests/mod.rs";
const GUARD_BYTES: &[u8] = include_bytes!("../../agent_reader_guard_digests/mod.rs");
...
fn receipt_matches(bytes: &[u8]) -> bool {
    ...
    rows.len() == ACCEPTED_CONTEXT_ARTIFACTS.len()
        && ACCEPTED_CONTEXT_ARTIFACTS.iter().all(|spec| {
            rows.iter()
                .filter(|row| {
...
fn accepted_context_intact(reads: &ReadSession, root: &Path) -> bool {
    let receipt = ArtifactSpec {
        path: READER_PROOF_PATH,
        sha256: READER_PROOF_SHA256,
    };
    // The frozen predecessor set is accepted historical context, not current reader proof. Its
    // unchanged receipt must still describe that exact artifact set,
