...
use super::fs::{check_symlink, physical_entry, relative};
use super::types::{
...
fn entry(
    ...
    physical_entry(
        ...
        root,
        path,
        format!("LEGACY-{}:{rel_text}", found.kind.to_ascii_uppercase()),
        &format!("legacy-{}-authority", found.kind),
        "OWN-MAINTENANCE",
        AuthorityState::Legacy,
        ActiveStatus::Active,
        None,
        Vec::new(),
        vec![format!("legacy-scope-evidence:{}", found.evidence)],
    )
}
