...
pub(crate) fn unknown_duplicate_ambiguous_missing_and_behavior_substitutions_fail_closed() {
    let unknown = String::from_utf8(VALID_CATALOG.to_vec())
        ...
        .into_bytes();
    let mut root = TestRoot::new("unknown-definition", &unknown);
    assert_eq!(
    ...
    let duplicate = String::from_utf8(VALID_CATALOG.to_vec())
        ...
        .into_bytes();
    let mut root = TestRoot::new("duplicate-definition", &duplicate);
    assert!(matches!(
    ...
    let ambiguous = String::from_utf8(VALID_CATALOG.to_vec())
        ...
        .replacen("\"node_id\": \"verify\"", "\"node_id\": \"Syntax\"", 1)
        .into_bytes();
    let mut root = TestRoot::new("ambiguous-definition", &ambiguous);
    assert_eq!(
    ...
    let mut root = TestRoot::new("missing-definition", &missing);
    assert_eq!(
    ...
    let mut root = TestRoot::new("behavior-substitution", &behavior);
    assert_eq!(
...
pub(crate) fn definition_candidate_input_and_dependency_drift_after_parse_are_refused() {
    let mut root = TestRoot::new("definition-drift", VALID_CATALOG);
    ...
    let mut root = TestRoot::new("candidate-drift", VALID_CATALOG);
    let catalog = load_full(&root, CANDIDATE_ID);
    assert_eq!(
        catalog
    ...
    let mut root = TestRoot::new("input-drift", VALID_CATALOG);
    let catalog = load_full(&root, CANDIDATE_ID);
    ...
    let mut root = TestRoot::new("dependency-omission", VALID_CATALOG);
    let catalog = load_full(&root, CANDIDATE_ID);
