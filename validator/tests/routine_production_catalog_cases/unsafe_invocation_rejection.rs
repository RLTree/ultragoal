...
pub(crate) fn execution_authority_fields_paths_and_prose_substitution_fail_closed() {
    for (field, value) in [
        ...
        let mut root = TestRoot::new(field, &bytes);
        assert_eq!(
    ...
    let mut root = TestRoot::new("overlapping-output", &bytes);
    assert_eq!(
    ...
    let mut root = TestRoot::new("read-path-escape", &bytes);
    assert_eq!(
        load_raw(&root, &bytes, one_node_adoption(&bytes)),
    ...
    let mut root = TestRoot::new("prose-substitution", &prose);
    assert_eq!(
    ...
    .unwrap();
    let mut root = TestRoot::new("digest-only-substitution", &digest_only);
    assert_eq!(
...
pub(crate) fn retired_v1_catalog_has_no_behavioral_reader() {
    ...
    let mut root = TestRoot::new("retired-v1", &legacy);
    assert_eq!(
...
pub(crate) fn symbolic_tool_executable_content_mode_and_identity_substitutions_are_refused() {
    let mut root = TestRoot::new("runner-substitution", VALID_CATALOG);
