...
use super::support::{AdapterMode, MockAdapter, TestDir, event, query, store, tree_snapshot};
...
#[test]
fn exclusive_contention_has_one_deadline_for_query_append_clear_recovery_and_export_reads() {
...
#[test]
fn same_process_identity_contention_times_out_every_store_path_without_hidden_writes() {
...
#[test]
fn identity_wait_and_file_wait_consume_one_shared_monotonic_budget() {
...
#[test]
fn shared_holder_allows_reads_but_exclusive_append_times_out_without_writing() {
...
#[test]
fn release_before_the_monotonic_deadline_succeeds_and_timeout_is_not_corruption() {
...
#[test]
fn identity_release_before_deadline_succeeds_and_cancelled_holder_leaves_no_lock() {
...
#[test]
fn product_sources_forbid_blocking_identity_locks_and_deadline_resets() {
    let identity = include_str!("../../src/observability/identity.rs");
    let locking = include_str!("../../src/observability/locking.rs");
    let store = include_str!("../../src/observability/store.rs");
...
fn assert_lock_timeout<T>(operation: impl FnOnce() -> Result<T, String>) {
    let started = Instant::now();
...
fn assert_identity_lock_timeout<T>(
    store: &EventStore,
...
fn assert_timeout_result<T>(result: Result<T, String>, elapsed: Duration) {
...
fn assert_order(source: &str, first: &str, second: &str, label: &str) {
    let first = source
        .find(first)
...
fn open_store_file(dir: &TestDir) -> File {
    OpenOptions::new()
        .read(true)
        .write(true)
        .open(dir.store_path())
...
fn locked_exclusive(dir: &TestDir) -> File {
