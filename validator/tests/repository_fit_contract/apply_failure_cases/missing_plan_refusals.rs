...
pub(crate) fn absent_plan(paths: &[&str]) -> (super::super::repository_fit::FitPlan, MemoryRepo) {
    let desired = desired(
        paths
            .iter()
...
pub(crate) fn apply_error(plan: &FitPlan, repo: &mut MemoryRepo) -> FitErrorId {
    apply(plan, &authorization(plan), repo).unwrap_err().id()
}

#[test]
pub(crate) fn first_write_then_error_restores_absence_and_prior_bytes() {
...
#[test]
pub(crate) fn later_write_then_error_restores_current_and_every_prior_mutation() {
...
#[test]
pub(crate) fn pre_write_error_restores_prior_mutations_only() {
    let (plan, mut repo) = absent_plan(&["A.md", "B.md"]);
...
#[test]
pub(crate) fn foreign_post_error_bytes_are_preserved_while_safe_prior_restores() {
    let (plan, mut repo) = absent_plan(&["A.md", "B.md"]);
    repo.error_after_cas = Some(2);
...
#[test]
pub(crate) fn reconciliation_read_failure_preserves_current_and_restores_prior() {
    let (plan, mut repo) = absent_plan(&["A.md", "B.md"]);
    repo.error_after_cas = Some(2);
...
#[test]
pub(crate) fn root_binding_change_after_effect_error_prevents_unsafe_rollback() {
...
#[test]
pub(crate) fn current_rollback_cas_race_does_not_skip_safe_prior_rollback() {
    let (plan, mut repo) = absent_plan(&["A.md", "B.md"]);
    repo.error_after_cas = Some(2);
...
#[test]
pub(crate) fn current_rollback_effect_error_does_not_skip_safe_prior_rollback() {
    let (plan, mut repo) = absent_plan(&["A.md", "B.md"]);
    repo.error_after_cas = Some(2);
...
#[test]
pub(crate) fn post_rollback_corruption_is_not_reported_as_an_ordinary_effect_failure() {
...
#[test]
pub(crate) fn prior_rollback_conflict_does_not_skip_older_safe_mutation() {
...
#[test]
pub(crate) fn prior_state_after_error_returns_original_failure_without_echoing_content() {
...
#[test]
pub(crate) fn final_sweep_rejects_raced_current_after_later_rollback() {
