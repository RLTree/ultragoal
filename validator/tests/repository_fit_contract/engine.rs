...
use super::support::{MemoryRepo, authorization, desired, file, managed_proof, sha};

#[test]
fn fresh_inspect_plan_apply_verify_reapply_and_rollback() {
...
#[test]
fn partial_retrofit_updates_only_known_generated_bytes() {
    ...
    let inspection = super::repository_fit::inspect_with_managed_proofs(
        FitMode::Retrofit,
        &desired,
        &[proof],
        &mut repo,
    ...
    assert!(matches!(
        ...
        super::repository_fit::OwnershipProvenance::AdoptedManifest { .. }
    ));
...
#[test]
fn known_generated_digest_without_opaque_state_proof_remains_a_conflict() {
    let prior = b"generated-v1\n";
    let desired = desired(vec![file(
        ".codex/generated.md",
        b"generated-v2\n",
...
#[test]
fn stale_or_unknown_managed_proof_is_rejected_not_silently_ignored() {
    let prior = b"generated-v1\n";
    let wanted = file(
        ".codex/generated.md",
        b"generated-v2\n",
    ...
    let stale = super::repository_fit::issue_managed_prior_proof(
        sha(b'a'),
        sha(b'b'),
        sha(b'd'),
        super::repository_fit::CanonicalPath::parse(".codex/generated.md").unwrap(),
        super::repository_fit::digest(prior),
    ...
    let mut repo = MemoryRepo::with(".codex/generated.md", prior);
    assert_eq!(
        super::repository_fit::inspect_with_managed_proofs(
            FitMode::Retrofit,
            &desired,
            &[stale],
            &mut repo,
...
#[test]
fn unknown_user_or_generated_edits_are_explicit_conflicts_and_never_write() {
...
#[test]
fn authorization_candidate_plan_and_root_are_exactly_bound() {
    ...
    .unwrap();
    let wrong =
        super::repository_fit::PlanAuthorization::new(sha(b'a'), sha(b'b'), sha(b'e')).unwrap();
    assert_eq!(
...
#[test]
fn compare_exchange_race_is_rejected_without_overwrite() {
    let desired = desired(vec![file("A.md", b"wanted", Ownership::UserOwned, &[])]);
    let mut repo = MemoryRepo::default();
    let plan = plan(
        &inspect(FitMode::Fresh, &desired, &mut repo).unwrap(),
...
#[test]
fn later_effect_failure_rolls_back_every_prior_mutation() {
    let desired = desired(vec![
...
#[test]
fn corrupted_effect_is_not_a_false_pass_and_failed_rollback_is_explicit() {
...
#[test]
fn inspect_plan_and_verify_are_zero_write_and_deterministic() {
    let desired = desired(vec![file("A.md", b"a", Ownership::UserOwned, &[])]);
...
#[test]
fn root_change_during_inspection_fails_closed() {
    let desired = desired(vec![file("A.md", b"a", Ownership::UserOwned, &[])]);
    let mut repo = MemoryRepo::default();
...
#[test]
fn already_matching_file_is_a_precondition_not_a_stale_plan_shortcut() {
    let desired = desired(vec![file("A.md", b"a", Ownership::UserOwned, &[])]);
    let mut repo = MemoryRepo::with("A.md", b"a");
    let plan = plan(
        &inspect(FitMode::Retrofit, &desired, &mut repo).unwrap(),
...
#[test]
fn external_change_to_matching_file_rolls_back_owned_mutations_only() {
    let desired = desired(vec![
        file("A.md", b"a", Ownership::UserOwned, &[]),
        file("B.md", b"b", Ownership::UserOwned, &[]),
    ]);
