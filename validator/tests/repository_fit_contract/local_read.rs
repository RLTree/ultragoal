...
use super::support::{desired, file};
use std::ffi::CString;
...
#[test]
fn anchored_local_inspect_plan_verify_are_recursive_tree_zero_write() {
...
#[test]
fn local_reader_rejects_exact_plus_ascii_casefold_aliases_at_every_component() {
...
#[test]
fn local_reader_distinguishes_alias_only_exact_only_and_safe_siblings() {
...
#[test]
fn local_reader_rejects_links_special_objects_and_traversal() {
...
#[test]
fn root_replacement_invalidates_open_reader_without_following_new_tree() {
...
#[test]
fn local_reader_enforces_requested_and_global_resource_limits() {
...
fn rejected(fixture: &Fixture, relative: &str) {
    let before = snapshot(&fixture.root);
    let mut reader = LocalRepository::open(&fixture.root).unwrap();
    assert_eq!(
        reader
...
fn create_distinct_file(path: &Path, bytes: &[u8]) -> bool {
...
fn create_distinct_directory(path: &Path) -> bool {
    ...
    fn visit(root: &Path, path: &Path, rows: &mut Vec<(String, String)>) {
        ...
        for entry in entries {
            ...
            let value = if metadata.is_dir() {
                ...
                format!(
                    "file:{}",
                    super::repository_fit::digest(&fs::read(&path).unwrap())
                )
