...
| --- | --- | --- | --- | --- |
| N06 routine execution | rework | `6f097aae5` / tree `89a0bdc6` | Exact-tree review found the held-lock control checks its deadline only after synchronous child completion, so a blocking regression hangs the proof; receipt and integration remain withheld | Enforce the contender deadline in the executable control, kill and reap only on timeout, and fail rather than accepting timeout |
...
current exact-candidate rejection count for N06 is `23`. The next action is the
bounded-proof invariant: the real contender control must itself return and fail
within its declared deadline even when the child blocks.
