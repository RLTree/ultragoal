...
| --- | --- | --- | --- | --- |
| N06 routine execution | rework | `05e3e20f8` / tree `711281ce` | Cumulative review proved crate-visible carried evidence can forge an already-observed cleanup, skip the real staged cleanup, and still release active authority; all higher claims remain withheld | Remove self-attested evidence from general errors; lifecycle-owned observation must perform cleanup and durable transition before returning or resuming |
...
current exact-candidate rejection count for N06 is `36`. The next action is the
shared evidence-custody repair: only the reservation lifecycle may observe
cleanup, authenticate the resulting record, and release authority; a general
error cannot carry a forgeable cleanup attestation. No receipt regeneration or
...
- Routine execution trust: active continuation thread
  ...
  exact clean source `05e3e20f8` / tree `711281ce` is in cumulative REWORK
  because crate-visible self-attested cleanup evidence bypasses actual cleanup;
  its stale WorkerResult, load-sensitive public result, and v1→v2 ledger
