...
| --- | --- | --- | --- | --- |
| N06 routine execution | rework | `18c4f3b97` / tree `59b49e0f` | Catalog construction claims its child scope before fallible setup, then panics without returning an owner or rolling back the partial fixture; source acceptance and receipt regeneration remain withheld | Make fixture construction a typed rollback transaction with substitution-safe failure controls, then refreeze once |
...
The next action is the N06 constructor-transaction repair. Escalation is legitimate only for
...
- N04 source transaction authority and its corrected WorkerResult are accepted,
  ...
  identity remain open. Root `d85fd050d` is clean. N06 `18c4f3b97` / tree
  `59b49e0f` remains the sole gate and is under REWORK because post-claim setup
  failure has no typed owner or checked rollback.
