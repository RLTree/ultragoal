...
| --- | --- | --- | --- | --- |
| N06 routine execution | rework | `6e2c42f5a` / tree `0ff55d0a` | Cumulative review proved a sibling can settle/release with staged executable custody still present, then return or panic through the settled fast path without cleanup; all higher claims remain withheld | Make terminal settlement refuse nonempty staged custody and remove direct caller mutation of settlement/custody state across every sibling |
...
current exact-candidate rejection count for N06 is `37`. The next action is the
shared terminal-custody repair: settlement and release must be impossible while
staged custody remains, and callers must not directly forge settled/custody
state. No receipt regeneration or dependent claim may consume this rejected
freeze.
...
- Routine execution trust: active continuation thread
  ...
  exact clean source `6e2c42f5a` / tree `0ff55d0a` is in cumulative REWORK
  because settlement can release while staged custody remains; its stale
