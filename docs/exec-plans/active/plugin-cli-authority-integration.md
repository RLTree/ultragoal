...
| --- | --- | --- | --- | --- |
| N06 routine execution | rework | `a20709a93` / tree `3345eb2c` | Exact-tree review found the timeout control can pass after failed kill and late natural reap; this is the second bounded-proof rejection, so receipt and integration remain withheld | Make verified kill-and-reap within an explicit cleanup bound the only constructible timeout-success state; classify failure and lateness separately |
...
current exact-candidate rejection count for N06 is `24`. The next action is the
shared bounded-child cleanup invariant across exit/deadline races, kill failure,
late or failed reap, pipe pressure, teardown, and holder release.
