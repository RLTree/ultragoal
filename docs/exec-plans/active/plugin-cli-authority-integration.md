...
- The disconnected legacy red execution engine and target-repository audit are
  ...
  reports 322 deny-warning errors, down from 664, with no unresolved reference
  ...
  clean compile. The latest bounded cleanup removed four definition-only schema
  adapters and unreachable scheduler variants/projections while preserving the
  live schema store and deterministic pure-read scheduler. This is not a
  compile pass; no suppression or dummy reachability is allowed.
