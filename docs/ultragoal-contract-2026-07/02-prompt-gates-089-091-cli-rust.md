...
- Generated, rebuildable, duplicated, superseded, cache-like, or stale runtime artifacts are deletion-first. Archiving is an exception only for protected proof, active release/review evidence, irreproducible raw evidence that has not yet been distilled into a receipt, legal/security retention, or an explicit user-approved retention exception.
- Archive exceptions must be typed, digest-bound, claim-limited, and justified with owner, retention reason, expiry or review condition, affected claim ids, and package-inclusion impact. Rebuildable caches, stale candidate archives, old review packets, abandoned worktree state, dependency installs, and generated temp artifacts cannot be retained merely to avoid deletion.
- GC validation and GC proof are distinct. Validation proves the cleanup mechanism, schemas, fixtures, and rejection paths are correct. Proof proves the current workspace cleanup result: classified inputs, protected set, plan digest, dry-run, apply receipt, reclaimed-byte accounting, remaining-byte summary, and post-delete verification on the current candidate. A valid dry-run, schema-valid receipt, or passing fixture alone cannot prove that the current workspace was cleaned.
...
- Red fixtures proving deletion without plan, deletion without dry-run, apply with wrong plan digest, protected artifact deletion, active-claim receipt deletion, unclassified artifact deletion, avoidable archive retention of rebuildable or superseded artifacts, archive exception without typed justification, missing reclaimed-byte accounting, dry-run substituted for actual cleanup proof, blind `rm -rf`, interrupted-agent cleanup without lock/pid/port/tempdir inspection, and deletion receipt without post-verify all fail GC law.
