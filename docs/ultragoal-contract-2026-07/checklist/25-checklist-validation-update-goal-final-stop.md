...
- [ ] Run standards enforcement proof. Evidence: Status: stale due to source change. Current source audit fails `agent-standards-enforcement` on downstream Product Usage/Product Cohesion evidence: fit-repo and plugin journey evidence are not same-candidate, and plugin-cohesion evidence is stale. Claim impact: no current standards-complete, Product Usage, Product Cohesion, readiness, release, completion, or `update_goal()` claim.
- [ ] Run foundational-law trace proof. Evidence: Status: stale due to source change. Current source-obligation aggregate remains blocked by downstream stale evidence and Gate 92/Product/Rust/GC families, not by exact coverage. Claim impact: no current foundational-trace-complete, readiness, release, completion, or `update_goal()` claim.
...
- [ ] Product Fitness receipt is current and substitution failures are enforced. Evidence: Status: stale due to source change; current Product/Fit/Journey receipts are not same-candidate. Claim impact: no readiness/release/completion/update_goal claim.
...
- [ ] Product Usage Fitness / routine CLI entrypoint is enforced. Evidence: Status: stale due to source change; routine usability needs same-candidate Product Usage proof before downstream claims. Claim impact: routine_usability only; no product-readiness, readiness, release, completion, final-packet, install/cache, registry/reviewer, or `update_goal()` claim.
...
- [ ] Fit-repo and target-repo plugin-activated usage paths are executable and discoverable through the CLI. Evidence: Status: stale due to source change; fit-repo and plugin journey evidence are not same-candidate. Claim impact: plugin-activated repo usability only; no readiness/release/completion claim.
...
- [ ] Targeted refactor/debt-removal/standards-gardener cadence proves repeated deviations and early debt are eliminated, mechanized, or claim-blocking. Evidence: Status: stale due to source change. Current source audit still reports standards-gardener candidate and changed-artifact mismatches. Claim impact: no current standards-gardener/debt-law closure claim.
- [ ] Plugin flow graph, package dependency closure, and plugin product journey receipt are enforced. Evidence: Status: stale due to source change; plugin journey receipt is not same-candidate even though the mandatory law surface is present.
