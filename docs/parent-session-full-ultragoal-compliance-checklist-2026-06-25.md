...
- [ ] Machine-readable observability fitting inventory tracks every law-bearing CLI command, validator check family, receipt/proof path, fixture/report path, package surface, and plugin surface as `fitted`, `partially_fitted`, or `unfitted`, names the current owner surface and next unfitted surface for each row, includes a validator-recomputed `fitting_control_board`, and the validator fails every partial, unfitted, missing, stale, adjacent-surface-substituted, count-mismatched, pass-shaped-control-board, or row-shape-only fitting row.
  - Status: stale due to source change for prior command query proofs. Source implementations remain in progress; full command/surface/loop/signal/validator-check/receipt/fixture/setup/long-running/external-live/claim-guard fitting remains incomplete.
...
- [ ] Run source/install/cache digest comparison. Evidence: Status: stale due to source change. Live source digest is `sha256:ebfcd0affdd432af3b13c6e2a4abbe498ced2892fdcf6c21e8c1605ed09495e6`; install/cache parity has not been refreshed for this source-local checkpoint. Claim impact: no disk source/install/cache parity, no app-registry/reviewer exposure, no final-packet correctness, no readiness, no release, no completion, and no `update_goal()` claim.
...
- [ ] Evidence path: Status: in progress. Current validated subpath: source-local research source-card anchor validator. Prior command/query/archive/coverage evidence is stale due to source change. Full Gate 92 remains blocked by exact coverage, full fitting, live-stack closure, source audit, red report, install/cache parity, final-packet correctness, and update_goal eligibility.
...
- [ ] Gate 92 evidence path: Status: in progress. Source-local research source-card anchor validation is current for digest `sha256:ebfcd0affdd432af3b13c6e2a4abbe498ced2892fdcf6c21e8c1605ed09495e6`; prior command/query proofs are stale due to source change. Full Gate 92 remains blocked until every command, plugin surface, operating-loop stage, signal row, and live-stack query path is fitted with same-candidate proof. Claim impact: no final packet correctness, review readiness, package readiness, release readiness, completion, `update_goal()` eligibility, registry exposure, reviewer exposure, install/cache parity, transactional finalization, or full Gate 92 proof.
...
- [ ] Agent-queryable observability status: stale due to source change for prior command/query/archive/coverage evidence; source-local research source-card anchor validation is current; remaining command/surface rows still partial or unfitted.
- [ ] Full local observability stack integration and non-opaque failure status: in progress; source-local research source-card anchor validation is current while exact coverage, full fitting, same-candidate command-query rebind, and live-stack closure remain blocked.
- [ ] Research source authority/article-to-law integration status: validated current for source-local research source-card anchor fail-closed validator; full Gate 93 remains in progress.
