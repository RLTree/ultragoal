...
- [ ] Gate 92 as the agent sensory system. Evidence: Status: validated current for touched `observe prove` production query/explain reconciliation; full Gate 92 remains in progress. Working means every law-bearing command/check provides useful logs, metrics, traces or wide events, eval linkage where behavioral quality is involved, bounded query proof, and explain output that says what failed, where, why, next repair, rerun command, and affected claims. Downstream impact: command inventory, fitting control board, telemetry schemas, receipts, fixtures, setup/retrofit, final packet, and update_goal blockers.
