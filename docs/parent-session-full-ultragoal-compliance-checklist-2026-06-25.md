...
- [ ] Gate 90 evidence path: Physical source topology repair is source-local for live implementation digest `sha256:e2e422756f4404c5e0d3f363d1172c4ee5aba891cd1aff2a02d6c8fb4a9abd41`: prefix-encoded moved modules include `validator/src/audit/package/run/mod.rs`, `validator/src/red/fixtures/{mod.rs,parallel.rs}`, `validator/src/scheduler/mod.rs`, and `validator/src/self_tests/receipts/validator/{mod.rs,boundaries.rs,generated.rs}`; focused namespace test passed and line-cap scan emitted no rows. Prior source-audit/coverage/red-report evidence is stale relative to this checkpoint; confidence calculation and dependent stop conditions remain unchecked.
