...
- [ ] Machine-readable observability fitting inventory tracks every law-bearing CLI command, validator check family, receipt/proof path, fixture/report path, package surface, and plugin surface as `fitted`, `partially_fitted`, or `unfitted`, names the current owner surface and next unfitted surface for each row, includes a validator-recomputed `fitting_control_board`, and the validator fails every partial, unfitted, missing, stale, adjacent-surface-substituted, count-mismatched, pass-shaped-control-board, or row-shape-only fitting row.
  - Status: validated current for source-local operating-loop research-input source-id and canonical URL enforcement on digest `sha256:152ef1c250bc0348319f77af328b7e9a19d10885a30ac32003ac92f189f347b4`. Prior command query proofs are stale due to source change; full fitting remains incomplete.
...
- [ ] Run Gate 92 runtime detection, stack up, stack health, stack smoke, query logs, query metrics, query traces, explain current failure, focused observability tests, observability red/green/tamper fixtures, exact coverage, line-cap scan, source audit, red fixture report, package digest, git status, and checkpoint commit. Evidence: Status: stale due to source change for prior coverage-prove query evidence; full Gate 92 fitting, source audit, red report, exact coverage, and live-stack closure remain pending.
- [ ] Run source/install/cache digest comparison. Evidence: Status: stale due to source change. Live source digest is `sha256:152ef1c250bc0348319f77af328b7e9a19d10885a30ac32003ac92f189f347b4`; install/cache parity has not been refreshed for this source-local checkpoint. Claim impact: no disk source/install/cache parity, no app-registry/reviewer exposure, no final-packet correctness, no readiness, no release, no completion, and no `update_goal()` claim.
...
- [ ] Evidence path: Status: in progress. Current validated subpaths: source-local research source-card anchor validator and operating-loop research-input source-id/canonical URL guard. Prior command/query/archive/coverage evidence is stale due to source change. Full Gate 92 remains blocked by exact coverage, full fitting, live-stack closure, source audit, red report, install/cache parity, final-packet correctness, and update_goal eligibility.
...
- [ ] Gate 92 evidence path: Status: in progress. Source-local research source-card anchor validation and operating-loop research-input inventory enforcement are current for digest `sha256:152ef1c250bc0348319f77af328b7e9a19d10885a30ac32003ac92f189f347b4`; prior command/query proofs are stale due to source change. Full Gate 92 remains blocked until every command, plugin surface, operating-loop stage, signal row, and live-stack query path is fitted with same-candidate proof. Claim impact: no final packet correctness, review readiness, package readiness, release readiness, completion, `update_goal()` eligibility, registry exposure, reviewer exposure, install/cache parity, transactional finalization, or full Gate 92 proof.
...
- [ ] Agent-queryable observability status: stale due to source change for prior command/query/archive/coverage evidence; source-local research source-card anchors and operating-loop research-input guard are current; remaining command/surface rows still partial or unfitted.
- [ ] Full local observability stack integration and non-opaque failure status: in progress; source-local research source-card anchors and operating-loop research-input guard are current while exact coverage, full fitting, same-candidate command-query rebind, and live-stack closure remain blocked.
