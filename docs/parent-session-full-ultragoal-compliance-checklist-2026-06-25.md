...
- [ ] Gate 92 evidence path: Status: in progress. `package digest` and `source audit` command fitting are validated current for source-local command-row proof; the first incomplete command row is `red fixture report`. Exact coverage remains stale due to source change and uncovered-code failure. Claim impact: no final packet correctness, review readiness, package readiness, release readiness, completion, `update_goal()` eligibility, registry exposure, reviewer exposure, install/cache parity, transactional finalization, or full Gate 92 proof.
...
- [ ] Agent-queryable observability status: in progress; `package digest` and `source audit` command-row fitting validated current, and fitted rows reject unbound red/green/tamper labels. `red fixture report` is the next incomplete command row.
