...
- [ ] Gate 5 status: stale after current source edits.
...
- [ ] Gate 5 evidence path: Exact coverage proof is stale after current source edits. Live source package digest is `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90` from `target/debug/ultragoal --root . package digest`; latest coverage receipt `validation_artifacts/coverage/coverage-receipt.json = sha256:dffd49b4c0ffca8ad4eca5b9b0da2ec85fe54cd6beba5a26e2e697f539e19c54` targets older digest `sha256:20f1637d41f593668d38620bb3452a258d30b1369d4a2c5a7f73a9d06153a756`. Claim impact: no current exact-coverage claim until coverage is rerun for the live digest; no readiness, release, final packet, registry/reviewer exposure, completion, or `update_goal()` claim.
...
- [ ] Default budget thresholds are enforced: `hot_edit_check` cold p95 <= 5 seconds; `focused_repair` cold p95 <= 15 seconds and warm p95 <= 5 seconds; `standard_source_local` cold p95 <= 30 seconds and warm p95 <= 10 seconds; `strict_local` p95 target <= 60 seconds; `strict_fixtures` full red/green/stale/wrong-surface/wrong-digest/tamper/substitute/miswire suite p95 target <= 60 seconds; `strict_coverage` exact coverage p95 target <= 60 seconds and hard ceiling <= 180 seconds; `strict_final` source-local p95 target <= 60 seconds and hard ceiling <= 180 seconds; `external_live` has typed bounded timeout/retry/backoff budgets, no unbounded execution, and claim blocking on failure.
  - Evidence: Partial current implementation adds scheduler/timing evidence for package schema checks and red-fixture evaluation. Red fixtures now split pure-read rows into parallel tasks and filesystem-mutating rows into serial authority tasks with deterministic aggregation. The full budget enum, full red/green/tamper performance fixture suite, performance receipts, and canonical source-audit evidence have not been regenerated for the current digest.
  - CLI command: `cargo test --offline performance --lib --quiet` passed 10/10, `cargo test --offline speed_budget --lib --quiet` passed 1/1, `cargo test --offline source_audit_parser_accepts_bounded_jobs --lib --quiet` passed 1/1, `cargo test --offline command_run_routes_audit_and_performance_variants --lib --quiet` passed 1/1, `cargo test --offline audit_run_rejects_bad_red_report_basename_and_malformed_target_receipts --lib --quiet` passed 1/1, `cargo test --offline package_checks --lib --quiet` passed 3/3, `cargo fmt --check` exited 0, and the line-cap scan emitted no rows.
  - Receipt: Not current; no Gate 89.20 completion receipt is supported by this row.
  - Candidate digest: live implementation digest is `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`; current performance receipt `validation_artifacts/cli/performance-receipt.json = sha256:f714eef6f78a9593dc2e3fb4c27cbeae71994778f89f97ab57ecbc3cb0428cd2` targets this digest, status `pass`, class `strict_local`, worker count `15`, queue depth `0`, and `supported_claim_classes = ["routine_usability"]`.
...
- [ ] Safe parallelism, multi-threading, and concurrency are mandatory defaults across the CLI, plugin, validator, fixtures, receipts, package scans, setup/retrofit, observability, shell helpers, and proof paths; any serial path has a typed authority-write, destructive/mutating, or external-constraint reason.
  - Evidence: partially implemented for the source-local scheduler and performance receipt path only. The universal CLI/plugin/shell/setup/retrofit fitting requirement remains unchecked until every command family and helper surface is inventoried, scheduler-routed or exception-typed, and validator-enforced.
  - CLI command:
  - Receipt:
  - Candidate digest: `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`
  - Status: partial source-local scheduler spine; not broad Gate 89.20 completion.
...
- [ ] Scheduler/executor task classes enforce `pure_read_parallel`, `isolated_temp_write_parallel`, `external_live_bounded_parallel`, `shared_authority_write_serial`, and `destructive_or_mutating_serial`, default worker count is `available_parallelism - 1` minimum `1`, `--jobs N` is bounded, results are deterministic, and workers cannot write shared `validation_artifacts/**`.
  - Evidence: current source-local implementation in `validator/src/scheduler/mod.rs` defines the typed task classes, `SchedulerConfig::default_jobs()` computes `available_parallelism - 1` with minimum `1`, `run_ordered()` preserves deterministic result order after parallel joins, and scheduler metrics record worker count/task count/queue depth/wall timing plus explicit unavailable CPU/memory/IO status. Focused tests passed: `cargo test --offline scheduler --lib --quiet` (`7/7`), `cargo test --offline package_checks_emit_bounded_scheduler_metrics_for_schema_phase --lib --quiet` (`1/1`), and `cargo test --offline performance --lib --quiet` (`10/10`). This row remains unchecked because not every CLI command and shell helper is scheduler-routed yet.
  - CLI command: `target/debug/ultragoal --root . performance prove --receipt validation_artifacts/cli/performance-receipt.json`
  - Receipt: `validation_artifacts/cli/performance-receipt.json = sha256:f714eef6f78a9593dc2e3fb4c27cbeae71994778f89f97ab57ecbc3cb0428cd2`
  - Candidate digest: `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`
  - Status: partial source-local scheduler/executor proof; not universal CLI/plugin fitting.
...
- [ ] Scheduler/performance evidence records worker count, task count, queue depth, wall time, CPU time when available, memory/IO when available, cache mode, resource-measurement status, candidate digest, and claim impact for every expensive law-bearing command.
  - Evidence: current performance receipt records `worker_count = 15`, `queue_depth = 0`, `cache_mode = disabled`, `target_revision.value = sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`, `claim_ceiling = performance_proven`, supported claims limited to `routine_usability`, and telemetry fields `wall_clock_ms`, `cpu_ms = null`, `peak_memory_bytes = null`, and `io_bytes = null`. Scheduler receipt fixture and package-check tests assert the resource-measurement status path. This row remains unchecked because every expensive law-bearing command is not yet fitted with this evidence.
  - CLI command: `target/debug/ultragoal --root . performance prove --receipt validation_artifacts/cli/performance-receipt.json`
  - Receipt: `validation_artifacts/cli/performance-receipt.json = sha256:f714eef6f78a9593dc2e3fb4c27cbeae71994778f89f97ab57ecbc3cb0428cd2`
  - Candidate digest: `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`
  - Status: partial source-local performance instrumentation proof; not universal command coverage.
...
- [ ] Coverage proof remains exactly 100 percent with `uncovered_records = []` after topology repair.
  - Evidence: stale after current source edits; broader Gate 90 topology/source-audit/readiness proof remains unchecked.
  - Command: `bash .harness/run-coverage.sh` has not been rerun for live package digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`.
  - Receipt: `validation_artifacts/coverage/coverage-receipt.json = sha256:dffd49b4c0ffca8ad4eca5b9b0da2ec85fe54cd6beba5a26e2e697f539e19c54`, `coverage.percent = 100`, `uncovered_records = []`, `claim_ceiling = supports_complete_coverage_claim`.
  - Candidate digest: `sha256:20f1637d41f593668d38620bb3452a258d30b1369d4a2c5a7f73a9d06153a756`.
  - Status: stale exact coverage receipt only; does not prove readiness, release, final packet correctness, registry/reviewer exposure, or `update_goal()` eligibility.
...
- [ ] `cargo llvm-cov` is the canonical Rust coverage proof path for declared Rust scope, with exact 100 percent coverage, `uncovered_records = []`, candidate/source/toolchain binding, and anti-gaming checks.
  - Evidence: exact coverage proof is stale after current source edits; broader Gate 91 Rust DevX/toolchain/anti-gaming proof remains unchecked.
  - CLI command: `bash .harness/run-coverage.sh` has not been rerun for live package digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`.
  - Receipt: `validation_artifacts/coverage/coverage-receipt.json = sha256:dffd49b4c0ffca8ad4eca5b9b0da2ec85fe54cd6beba5a26e2e697f539e19c54`, `coverage.percent = 100`, `uncovered_records` count `0`, `claim_ceiling = supports_complete_coverage_claim`.
  - Candidate digest: `sha256:20f1637d41f593668d38620bb3452a258d30b1369d4a2c5a7f73a9d06153a756`.
  - Status: exact coverage proof is stale; broader Gate 91, readiness, release, final packet, registry/reviewer, or `update_goal()` claim is not supported by this row.
...
- [ ] Agent proof comes from CLI queries against VictoriaLogs, VictoriaMetrics, and VictoriaTraces; Grafana inspection is not claim authority.
  - Evidence: current fail-closed `observe prove` run `run-f350b72154418fef7c5b00b1a1c7addc66a7e4511f612f59500d5bb3e54a435b` was queried through CLI against VictoriaLogs, VictoriaMetrics, and VictoriaTraces for candidate `sha256:96a82b708ccd877874690d2c29cb78f06967b7f5ddb897be5bc27ac538837641`; all three query commands exited `0`. The query receipts promote `observed_failure_class = observability_gate_failure`, `observed_where_failed = observe.prove`, and same-digest row metadata; logs and traces carry full `observed_why_failed = live observability stack is not health-checked and smoke-proven`, while metrics carry the bounded sanitized label value. This proves queryability for this current failure only, not full command, plugin surface, operating-loop, or signal fitting.
  - Query commands: `target/debug/ultragoal --root . observe logs query --run-id run-f350b72154418fef7c5b00b1a1c7addc66a7e4511f612f59500d5bb3e54a435b --limit 100 --receipt validation_artifacts/observability/observe-prove-logs-query.json`; `target/debug/ultragoal --root . observe metrics query --run-id run-f350b72154418fef7c5b00b1a1c7addc66a7e4511f612f59500d5bb3e54a435b --limit 100 --receipt validation_artifacts/observability/observe-prove-metrics-query.json`; `target/debug/ultragoal --root . observe traces query --run-id run-f350b72154418fef7c5b00b1a1c7addc66a7e4511f612f59500d5bb3e54a435b --limit 100 --receipt validation_artifacts/observability/observe-prove-traces-query.json`.
  - Receipts: `validation_artifacts/observability/observe-prove-logs-query.json = sha256:adc02d21181e847db1f0a86dab464a68261df5400c5be8dda2d81bbca18a4f0c`; `validation_artifacts/observability/observe-prove-metrics-query.json = sha256:79541f370861d29a3f0a4b536088ce2dc0e22995e448858de38573e88f16971a`; `validation_artifacts/observability/observe-prove-traces-query.json = sha256:25701577f425c0e4df2e8b8ad5354aac44cf75ba15a33da75946563ab464cc51`.
  - Candidate digest: `sha256:96a82b708ccd877874690d2c29cb78f06967b7f5ddb897be5bc27ac538837641`.
...
- [ ] Required query proof covers failed run by run_id, failed law by law_id, failed check by check_id, blocked claim by claim_id, command duration metrics, stale receipt counters, full command trace, and current proof-graph failure across logs, metrics, and traces.
  ...
  - Query results: current run `run-f350b72154418fef7c5b00b1a1c7addc66a7e4511f612f59500d5bb3e54a435b` returns current-digest rows in `observe-prove-logs-query.json`, `observe-prove-metrics-query.json`, and `observe-prove-traces-query.json`, each with typed observed failure fields.
  - Receipt: `validation_artifacts/observability/observe-explain-failure.json = sha256:b606d1ed6fead2de04718907004a888cc69d694c926b64e38d1176818aa76a27`; `why_failed`, `where_failed`, `next_repair`, `law_id`, `check_id`, `claim_id`, `claim_impact`, and query hints all describe the failed `observe.prove` run, not the successful explanation wrapper.
  - Candidate digest: `sha256:96a82b708ccd877874690d2c29cb78f06967b7f5ddb897be5bc27ac538837641`.
...
- [ ] The current proof-graph failure is visible through stdout, source audit receipt, final-packet proof receipt, VictoriaLogs, VictoriaMetrics, VictoriaTraces, observe query commands, and `observe explain-failure --run-id`.
  - Evidence: current source-local failure is visible through `observe prove` stdout and current observability proof `validation_artifacts/observability/observe-prove.json = sha256:312e4467822afe3d897cd081119231fda99d402acc635fcb451c8c1c47449916`. The command exited `1` and printed `failed_check=full-local-observability-stack-integration-non-opaque-failure`, `why=live observability stack is not health-checked and smoke-proven`, `where=observe.prove`, claim impact, `next_repair=run observe stack health and observe stack smoke`, run id, correlation id, and exact observe query commands. Logs/metrics/traces/explain receipts are current for the same run and promote the observed failure details into typed fields. Source audit, red fixture report, final-packet proof, source audit receipt visibility, and final-packet proof receipt visibility remain stale or unsupported for this digest.
  - Run id: `run-f350b72154418fef7c5b00b1a1c7addc66a7e4511f612f59500d5bb3e54a435b`; correlation id `corr-4624b767c63c08563939c01a630ed03000970937fb94c9942746099f29636b8f`.
  - Query receipts: `validation_artifacts/observability/observe-prove-logs-query.json = sha256:adc02d21181e847db1f0a86dab464a68261df5400c5be8dda2d81bbca18a4f0c`, `validation_artifacts/observability/observe-prove-metrics-query.json = sha256:79541f370861d29a3f0a4b536088ce2dc0e22995e448858de38573e88f16971a`, `validation_artifacts/observability/observe-prove-traces-query.json = sha256:25701577f425c0e4df2e8b8ad5354aac44cf75ba15a33da75946563ab464cc51`, and `validation_artifacts/observability/observe-explain-failure.json = sha256:b606d1ed6fead2de04718907004a888cc69d694c926b64e38d1176818aa76a27`.
  - Candidate digest: `sha256:96a82b708ccd877874690d2c29cb78f06967b7f5ddb897be5bc27ac538837641`.
...
- [ ] Status: in_progress
- [ ] Run `cargo fmt --check`. Evidence: command exited `0` for current source-local package digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`. Claim impact: formatting check only; no install/cache parity, final packet correctness, registry/reviewer exposure, readiness, release, completion, or `update_goal()` claim.
- [ ] Run `cargo test --offline`. Evidence: full `cargo test --offline` has not been rerun for current source-local package digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`; current inner-loop evidence is `cargo test --offline --lib --quiet`, which exited `0` with `570/570` library tests passing. Claim impact: source-local Rust library test proof only; no install/cache parity, final-packet correctness, registry/reviewer exposure, readiness, release, completion, or `update_goal()` claim.
- [ ] Run full source audit with receipt and red fixture report. Evidence: unchecked and stale. Current live source-local package digest is `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`; latest persisted source audit targets `sha256:1bf81589063cd172cf8a896594ccc2c75f53924b04dd90cb7c2596fb65cd42f5`, records `status = fail`, and has run id `ultragoal-audit-2026-06-30T19:02:13Z`; latest persisted red fixture report also targets `sha256:1bf81589063cd172cf8a896594ccc2c75f53924b04dd90cb7c2596fb65cd42f5` and records `status = fail`. Claim impact: no current source-audit or red-report pass claim; no registry/reviewer exposure, final-packet correctness, readiness, release, completion, or `update_goal()` claim.
...
- [ ] Run coverage command proving 100%. Evidence: stale after current source edits. Live source package digest is `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`; latest coverage receipt targets older digest `sha256:20f1637d41f593668d38620bb3452a258d30b1369d4a2c5a7f73a9d06153a756` even though it records `coverage.percent = 100` and `uncovered_records = []`. Claim impact: no current exact-coverage claim until coverage is rerun for the live digest; no readiness/release/completion/final-packet/registry/update_goal claim.
...
- [ ] Run line-cap command. Evidence: `find validator/src -name '*.rs' -exec wc -l {} + | awk '$2 != "total" && $1 > 250 { print }'` exited `0` and emitted no over-cap Rust files for package digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`. Claim impact: line-cap observation only; broader line-cap law and readiness/release/completion/update_goal claims remain unchecked.
...
- [ ] Evidence path: Current source-local implementation digest is `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90` from `target/debug/ultragoal --root . package digest`. Current stabilization evidence is limited to package digest, current exact coverage receipt, current performance-only receipt, `cargo fmt --check`, `cargo test --offline --lib --quiet` with `570/570` passing, focused scheduler/package/performance/final-packet/control-plane/red-identity tests, `cargo build --offline --quiet`, and a raw line-cap scan. Product/Fit/Journey, Rust/GC, standards-gardener, source audit, red report, final-packet, registry/app/install/cache, self-law, and update-goal receipts are stale or not rerun for this digest. Claim impact: source-local evidence only; current coverage proof supports exact plugin self-coverage only and current performance proof supports routine-usability/performance only; no final-packet correctness, transactional finalization green proof, CLI self-law green proof, update_goal eligibility, registry/reviewer exposure, readiness, release, completion claim, install/cache parity, or `update_goal()` call.

## `update_goal()` Is Forbidden Until All Are True

- [ ] Status: in_progress
- [ ] Current source audit passes.
  - Evidence: unchecked. Latest persisted source audit is stale and failing relative to live implementation digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`; `validation_artifacts/ultragoal-audit/validator-receipt.json` targets `sha256:1bf81589063cd172cf8a896594ccc2c75f53924b04dd90cb7c2596fb65cd42f5`, status `fail`, run id `ultragoal-audit-2026-06-30T19:02:13Z`. Claim impact: no current source-audit pass claim; readiness, release, completion, final-packet correctness, and `update_goal()` remain blocked.
- [ ] Current red fixture report passes with all fixtures failing for intended reasons.
  - Evidence: unchecked. Latest persisted red fixture report is stale and failing relative to live implementation digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`; `validation_artifacts/ultragoal-audit/red-fixture-report.json` targets `sha256:1bf81589063cd172cf8a896594ccc2c75f53924b04dd90cb7c2596fb65cd42f5` and status `fail`, so it is not current proof. Claim impact: no current red-fixture pass claim, no source-audit pass, readiness, release, completion, final-packet correctness, or `update_goal()` claim.
- [ ] No standards law remains optional or unmechanized for material claims. Evidence: unchecked. Latest standards/source-audit/standards-gardener evidence targets prior package digests, including `sha256:1bf81589063cd172cf8a896594ccc2c75f53924b04dd90cb7c2596fb65cd42f5`, and is stale relative to live implementation digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`. Claim impact: no current standards-complete claim.
- [ ] Foundational article law trace is complete and validator-enforced with no weak/deferral escape hatch.
  - Evidence: unchecked. Latest foundational trace/source-obligation/source-audit evidence targets prior package digest `sha256:1bf81589063cd172cf8a896594ccc2c75f53924b04dd90cb7c2596fb65cd42f5` or older and is stale relative to live implementation digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`. Claim impact: no current foundational-trace-complete claim.
- [x] Plugin self-coverage is 100% with typed receipt and no uncovered records.
  - Evidence: current exact coverage proof targets live source package digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`. `bash .harness/run-coverage.sh` exited `0`; `validation_artifacts/coverage/coverage-receipt.json = sha256:e5bd96bbfbe3e297faf6e4a2382ad097ac08cc481390584cd42f4325ac5ed847` records `coverage.percent = 100`, `uncovered_records = []`, and `claim_ceiling = supports_complete_coverage_claim`. Supporting artifacts: `validation_artifacts/coverage/llvm-cov-full.json = sha256:738addc471013049b108e89d8467e38d7a18d222769df6b3053e41517a8d0ea4`; `validation_artifacts/coverage/missing-lines.txt = sha256:29701803401f18dc0e17d52152a9c00328840502907cf3c56be84cbaa0cf0ba2`. Claim impact: exact plugin self-coverage only; no readiness/release/final-packet/registry/reviewer/completion or `update_goal()` claim.
...
- [ ] Line-cap adherence is enforced. Evidence: current source-local line-cap observation is clean for package digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`: `find validator/src -name '*.rs' -exec wc -l {} + | awk '$2 != "total" && $1 > 250 { print }'` emitted no rows, `cargo fmt --check` exited `0`, and focused scheduler/package/performance/final-packet/control-plane tests plus full library tests passed. This row remains unchecked because durable current source-audit/receipt-bound line-cap closure has not been rerun for this digest. Claim impact: source-local line-cap/source-topology evidence only; no readiness/release/completion/update_goal claim.
...
- [ ] Product Fitness receipt is current and substitution failures are enforced. Evidence: unchecked. Latest persisted Product/Fit/Journey receipts target `sha256:84c0e8f27dabfa79804b516d9606670ba235b9d994b5636f53f230d21be08add`, not live implementation digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`; Product/Fit/Journey receipts must be rebound before any current source-local Product/Fit/Journey claim. Claim impact: no current Product/Fit/Journey claim; no readiness/release/final-packet/registry/reviewer/completion or `update_goal()` claim.
...
- [ ] Gate 92 evidence path: In progress, source-local only. Latest live implementation digest is `sha256:e2e422756f4404c5e0d3f363d1172c4ee5aba891cd1aff2a02d6c8fb4a9abd41`; prior Gate 92 stack/query/prove/explain receipts target older digests and are stale until rebound. The fitting inventory still blocks full Gate 92 because not every command, plugin surface, operating-loop stage, or signal row is fitted with same-candidate query proof. Claim impact: no final packet correctness, review readiness, package readiness, release readiness, completion, `update_goal()` eligibility, registry exposure, reviewer exposure, or full Gate 92 proof.
