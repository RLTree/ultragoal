...
- [ ] Coverage proof remains exactly 100 percent with `uncovered_records = []` after topology repair.
  - Evidence: Status: stale due to source change. Current exact coverage proof is red with one uncovered record in `validator/src/audit/observability/registry/fitting.rs`.
  - Status: stale due to source change; no readiness, release, final packet correctness, registry/reviewer exposure, or `update_goal()` eligibility.
...
- [ ] `cargo llvm-cov` is the canonical Rust coverage proof path for declared Rust scope, with exact 100 percent coverage, `uncovered_records = []`, candidate/source/toolchain binding, and anti-gaming checks.
  - Evidence: Status: stale due to source change. Current exact coverage proof is red with one uncovered record in `validator/src/audit/observability/registry/fitting.rs`.
  - Status: stale due to source change; broader Gate 91, readiness, release, final packet, registry/reviewer, or `update_goal()` claim is not supported by this row.
...
- [ ] Machine-readable observability fitting inventory tracks every law-bearing CLI command, validator check family, receipt/proof path, fixture/report path, package surface, and plugin surface as `fitted`, `partially_fitted`, or `unfitted`, names the current owner surface and next unfitted surface for each row, includes a validator-recomputed `fitting_control_board`, and the validator fails every partial, unfitted, missing, stale, adjacent-surface-substituted, count-mismatched, pass-shaped-control-board, or row-shape-only fitting row.
  ...
  - Next blocker: first incomplete row is `product prove-fitness` pass stdout contract; other command, surface, loop, and signal rows remain partial or unfitted.
...
- [ ] Run coverage command proving 100%. Evidence: Status: stale due to source change. Current exact coverage proof is red with one uncovered record in `validator/src/audit/observability/registry/fitting.rs`. Claim impact: no coverage, readiness, release, completion, final-packet, registry/reviewer, or `update_goal()` claim.
...
- [ ] Plugin self-coverage is 100% with typed receipt and no uncovered records.
  - Evidence: Status: stale due to source change. Current exact coverage proof is red with one uncovered record in `validator/src/audit/observability/registry/fitting.rs`; no readiness/release/completion/final-packet/registry/update_goal claim.
