...
- [ ] Safe parallelism, multi-threading, and concurrency are mandatory defaults across the CLI, plugin, validator, fixtures, receipts, package scans, setup/retrofit, observability, shell helpers, and proof paths; any serial path has a typed authority-write, destructive/mutating, or external-constraint reason.
  - Evidence: Status: implemented, pending validation for the source-local scheduler/performance slice only. Universal CLI/plugin/shell/setup/retrofit fitting is still in progress.
  - CLI command:
  - Receipt:
  - Candidate digest: `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`
  - Status: in progress.
...
- [ ] Scheduler/executor task classes enforce `pure_read_parallel`, `isolated_temp_write_parallel`, `external_live_bounded_parallel`, `shared_authority_write_serial`, and `destructive_or_mutating_serial`, default worker count is `available_parallelism - 1` minimum `1`, `--jobs N` is bounded, results are deterministic, and workers cannot write shared `validation_artifacts/**`.
  - Evidence: Status: implemented, pending validation. Source-local scheduler has typed task classes, default jobs, deterministic ordered joins, and no shared artifact worker-write policy; focused scheduler/package/performance tests pass. Row remains unchecked until every CLI command and shell helper is fitted.
...
- [ ] Scheduler/performance evidence records worker count, task count, queue depth, wall time, CPU time when available, memory/IO when available, cache mode, resource-measurement status, candidate digest, and claim impact for every expensive law-bearing command.
  - Evidence: Status: implemented, pending validation for `performance prove` and scheduler-backed package checks. Every expensive law-bearing command is not fitted yet.
  - CLI command: `target/debug/ultragoal --root . performance prove --receipt validation_artifacts/cli/performance-receipt.json`
  - Receipt: `validation_artifacts/cli/performance-receipt.json = sha256:f714eef6f78a9593dc2e3fb4c27cbeae71994778f89f97ab57ecbc3cb0428cd2`
  - Candidate digest: `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`
  - Status: implemented, pending validation.
...
- [x] Plugin self-coverage is 100% with typed receipt and no uncovered records.
  - Evidence: Status: validated current. Coverage receipt targets `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90` with `coverage.percent = 100` and `uncovered_records = []`. Claim impact: exact plugin self-coverage only; no readiness/release/final-packet/registry/reviewer/completion or `update_goal()` claim.
...
- [ ] Product Fitness receipt is current and substitution failures are enforced. Evidence: Status: implemented, pending validation. Product/Fit/Journey receipts have been rebound to live implementation digest `sha256:fd5f598f0a70a1fc4128bf29eae3f73ced9d5ec0bdce3a8b0a144942549b3a90`, and focused product tests passed. Row remains unchecked until source audit/red fixture validation is current. Claim impact: source-local Product/Fit/Journey only; no readiness/release/final-packet/registry/reviewer/completion or `update_goal()` claim.
