...
- [ ] Gate 92 evidence path: Status: in progress. Command-row fitting is validated current through `source-obligations check`; the first incomplete command row is `foundational-trace check`. Source-obligations validation still fails source-local on stale foundational trace/source bindings; exact coverage, source audit, and red fixture report remain stale/failing, and full Gate 92 remains blocked. Claim impact: no final packet correctness, review readiness, package readiness, release readiness, completion, `update_goal()` eligibility, registry exposure, reviewer exposure, install/cache parity, transactional finalization, or full Gate 92 proof.
...
- [ ] Agent-queryable observability status: in progress; command-row fitting validated current through `source-obligations check`, and fitted rows reject unbound red/green/tamper labels. `foundational-trace check` is the next incomplete command row.
