...
- [ ] The current proof-graph failure is visible through stdout, source audit receipt, final-packet proof receipt, VictoriaLogs, VictoriaMetrics, VictoriaTraces, observe query commands, and `observe explain-failure --run-id`.
  - Status: in progress; touched `observe prove` fitting-board failure is visible through stdout, logs, metrics, traces, and explain-failure.
  - Next blocker: first stale query proof is `package digest`; control-board first incomplete command row is `line-cap check`.
...
- [ ] Gate 92 evidence path: Status: in progress. Touched observe and coverage failure paths now produce agent-legible stdout, bounded metric signals, same-candidate log/trace query proof, and non-fallback explain output. Exact coverage still fails source-local on uncovered code, source audit and red fixture report remain stale/failing, and full Gate 92 remains blocked by partial command/surface/signal/eval fitting. Claim impact: no final packet correctness, review readiness, package readiness, release readiness, completion, `update_goal()` eligibility, registry exposure, reviewer exposure, install/cache parity, transactional finalization, or full Gate 92 proof.
...
- [ ] Gate 92 as the agent sensory system. Evidence: Status: validated current for touched `observe prove` fitting-board failure output/query/explain reconciliation; full Gate 92 remains in progress. Working means every law-bearing command/check provides useful logs, metrics, traces or wide events, eval linkage where behavioral quality is involved, bounded query proof, and explain output that says what failed, where, why, next repair, rerun command, and affected claims. Downstream impact: command inventory, fitting control board, telemetry schemas, receipts, fixtures, setup/retrofit, final packet, and update_goal blockers.
