...
- [ ] Machine-readable operating-loop and signal inventory tracks whether observability is actually usable as the repair loop: current digest first, failing command capture, logs/metrics/traces query by run id, CLI explanation before manual artifact inspection, smallest repair, narrow rerun, before/after telemetry comparison, broad-audit gating, freshness, and the CLI-translated latency/traffic/error/saturation/freshness/correlation/redaction/boundedness signal model.
  - Status: in progress; same-candidate query/explain loop is validated current for the mandatory-law validation slice and observe/prove claim guard.
...
- [ ] The current proof-graph failure is visible through stdout, source audit receipt, final-packet proof receipt, VictoriaLogs, VictoriaMetrics, VictoriaTraces, observe query commands, and `observe explain-failure --run-id`.
  - Status: in progress; current observe/prove failure is visible through stdout, logs, metrics, traces, and explain-failure.
...
- [ ] Run coverage command proving 100%. Evidence: Status: validated current for exact coverage only. Claim impact: no readiness/release/completion/final-packet/registry/update_goal claim.
...
- [ ] Run agent-queryable observability proof and red fixtures. Evidence: Status: in progress. Coverage logs query and explain output are current; metrics/traces query fitting remains incomplete and full Gate 92 remains blocked.
...
- [ ] Run Gate 92 runtime detection, stack up, stack health, stack smoke, query logs, query metrics, query traces, explain current failure, focused observability tests, observability red/green/tamper fixtures, exact coverage, line-cap scan, source audit, red fixture report, package digest, git status, and checkpoint commit. Evidence: Status: in progress; exact coverage, coverage-prove, line-cap scan, namespace scan, product journey telemetry queries, and focused observability tests are validated current for this source-local slice. Full Gate 92 inventory, source audit, red report, and live-stack closure remain pending.
...
- [ ] Evidence path: Status: in progress. Validated current subpaths: package-digest command/query proof, line-cap command/query fitting, namespace command fitting, product prove-journey command/query fitting, source-audit runtime/guard/parser telemetry, target-repo parser telemetry, red fixture report standalone command telemetry, schema validation command telemetry, mandatory-law validation command telemetry, standards-gardener rebind command telemetry, stale red-report fail-closed guard, fail-closed command-fitting semantics, and observe/prove first command blocker moved to source-obligations check. Full Gate 92 remains in progress; no final-packet correctness, CLI self-law green proof, update_goal eligibility, registry/reviewer exposure, readiness, release, completion claim, install/cache parity, or `update_goal()` call.
...
- [ ] Plugin self-coverage is 100% with typed receipt and no uncovered records.
  - Evidence: Status: validated current for exact coverage only; no readiness/release/completion/final-packet/registry/update_goal claim.
...
- [ ] Observability-led narrow repair loop is enforced before broad audit reruns. Evidence: Status: validated current for the coverage failure/query/explain/repair/rerun loop and namespace module-boundary repair rerun in this slice. Full loop enforcement across every law-bearing command remains incomplete. Claim impact: no broad-boundary closure claim without narrow failure/query/explain/repair proof.
...
- [ ] Agent-queryable observability status: in progress; research binding, inventory row-accounting enforcement, line-cap command/query fitting, namespace command/query fitting, product prove-fitness command/query fitting, product prove-cohesion command/query fitting, product prove-journey command/query fitting, and prior focused command fitting are validated current.
- [ ] Full local observability stack integration and non-opaque failure status: in progress; research binding, inventory/control-board enforcement, line-cap command/query fitting, namespace command/query fitting, product prove-fitness command/query fitting, product prove-cohesion command/query fitting, and product prove-journey command/query fitting are validated current while full fitting remains blocked.
