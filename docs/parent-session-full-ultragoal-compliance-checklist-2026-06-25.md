...
- [ ] Gate 92 as the agent sensory system. Evidence: Status: in progress. Working means every law-bearing command/check provides useful logs, metrics, traces or wide events, eval linkage where behavioral quality is involved, bounded query proof, and explain output that says what failed, where, why, next repair, rerun command, and affected claims. Downstream impact: command inventory, fitting control board, telemetry schemas, receipts, fixtures, setup/retrofit, final packet, and update_goal blockers.
...
- [ ] Agent State / Next Action Contract. Evidence: Status: not started. Working means `ultragoal next` and `ultragoal next --json` tell an un-oriented agent or Tree the current candidate digest, dirty/stale state, active phase, strict claim ceiling, first legal blocker, why it comes first, dependency chain, stale or wrong-digest evidence, exact next repair, exact narrow rerun, observe query/explain commands when available, broad rerun only when allowed, forbidden actions, Gate 92 fitting summary, Product Usage status, worktree eligibility, and Agent Cockpit state. Done should feel like the harness answers "where am I, what is blocked, why, what proves the next repair, and what claims are forbidden" without manual archaeology through parent prompts, stale checklist rows, raw receipts, command-inventory walls, or source-audit dumps. Validation means parser/help/schema/bounds/redaction/fixture mechanics pass; proof means a real current-candidate command run, same-candidate receipt/stdout/JSON/logs/metrics/traces/explain reconciliation, and source inspection prove the output is typed-authority driven rather than checklist prose or last-command heuristics. Downstream impact: Product Usage Fitness, Agent Cockpit, Gate 105, final packet product shape, worktree eligibility, and update_goal blockers.
