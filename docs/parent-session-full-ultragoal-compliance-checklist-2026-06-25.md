...
- [ ] Machine-readable observability fitting inventory tracks every law-bearing CLI command, validator check family, receipt/proof path, fixture/report path, package surface, and plugin surface as `fitted`, `partially_fitted`, or `unfitted`, names the current owner surface and next unfitted surface for each row, includes a validator-recomputed `fitting_control_board`, and the validator fails every partial, unfitted, missing, stale, adjacent-surface-substituted, count-mismatched, pass-shaped-control-board, or row-shape-only fitting row.
  - Status: in progress. Current first incomplete row is `review-round verify`; `fit-repo prove` is validated current for source-local command/query fitting. `product prove-cohesion` and `product prove-journey` remain implemented/fitted, but their same-candidate query receipts are stale due to source change and need boundary rebind. Other command, surface, loop, signal, validator-check, receipt, fixture, setup, long-running, external/live, and claim-guard rows remain partial or unfitted.
...
- [ ] The current proof-graph failure is visible through stdout, source audit receipt, final-packet proof receipt, VictoriaLogs, VictoriaMetrics, VictoriaTraces, observe query commands, and `observe explain-failure --run-id`.
  ...
  - Next blocker: `review-round verify` is the first incomplete command row.
...
- [ ] Evidence path: Status: in progress. Validated current subpaths: package-digest command/query proof, line-cap command/query fitting, namespace command fitting, `fit-repo prove` command/query fitting, source-audit runtime/guard/parser telemetry, target-repo parser telemetry, red fixture report standalone command telemetry, schema validation command telemetry, mandatory-law validation command telemetry, standards-gardener rebind command telemetry, stale red-report fail-closed guard, and fail-closed command-fitting semantics. `product prove-cohesion` and `product prove-journey` remain implemented/fitted, but their same-candidate query receipts are stale due to source change and need boundary rebind. Current next command row is `review-round verify`. Full Gate 92 remains in progress; no final-packet correctness, CLI self-law green proof, update_goal eligibility, registry/reviewer exposure, readiness, release, completion claim, install/cache parity, or `update_goal()` call.
...
- [ ] Agent-queryable observability status: in progress; research binding, row-contract inventory enforcement, control-board recomputation, and `fit-repo prove` command/query fitting are validated current. Earlier product command query receipts are stale due to source change and need boundary rebind; remaining command/surface rows still partial or unfitted.
