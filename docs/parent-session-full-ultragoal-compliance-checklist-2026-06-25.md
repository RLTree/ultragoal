...
- [ ] Safe parallelism, multi-threading, and concurrency are mandatory defaults across the CLI, plugin, validator, fixtures, receipts, package scans, setup/retrofit, observability, shell helpers, and proof paths; any serial path has a typed authority-write, destructive/mutating, or external-constraint reason.
  - Evidence: Status: implemented, pending validation for the source-local scheduler/package-check/performance slice only. Universal CLI/plugin/shell/setup/retrofit fitting is still in progress.
  - CLI command:
  - Receipt:
  - Candidate digest: stale due to source change; next phase-boundary digest pending.
  - Status: in progress.
...
- [ ] Scheduler/executor task classes enforce `pure_read_parallel`, `isolated_temp_write_parallel`, `external_live_bounded_parallel`, `shared_authority_write_serial`, and `destructive_or_mutating_serial`, default worker count is `available_parallelism - 1` minimum `1`, `--jobs N` is bounded, results are deterministic, and workers cannot write shared `validation_artifacts/**`.
  - Evidence: Status: implemented, pending validation. Source-local scheduler has typed task classes, default jobs, deterministic ordered joins, no shared artifact worker-write policy, red-fixture fan-out, and package-check fan-out; focused scheduler/package/audit tests pass. Row remains unchecked until every CLI command and shell helper is fitted.
...
- [ ] Scheduler/performance evidence records worker count, task count, queue depth, wall time, CPU time when available, memory/IO when available, cache mode, resource-measurement status, candidate digest, and claim impact for every expensive law-bearing command.
  - Evidence: Status: implemented, pending validation for `performance prove`, scheduler-backed package checks, and red-fixture scheduler metrics. Every expensive law-bearing command is not fitted yet.
  - CLI command: `target/debug/ultragoal --root . performance prove --receipt validation_artifacts/cli/performance-receipt.json`
  - Receipt: `validation_artifacts/cli/performance-receipt.json = sha256:f714eef6f78a9593dc2e3fb4c27cbeae71994778f89f97ab57ecbc3cb0428cd2`
  - Candidate digest: stale due to source change; next phase-boundary digest pending.
  - Status: implemented, pending validation.
...
- [ ] Evidence path: Status: in progress. Current live implementation digest is `sha256:895cdad77175a1041ce30e921327e10b74b2170441bbb961d6da01d77537cd08`. The package-check scheduler slice is implemented and pending phase-boundary validation; focused package scheduler tests, focused red-fixture tests, `cargo fmt --check`, build, diff whitespace check, and raw line-cap scan passed. Phase-boundary coverage, Product/Fit/Journey, Rust/GC, standards-gardener, source audit, red report, final-packet, registry/app/install/cache, self-law, and update-goal receipts are stale or pending. Claim impact: source-local implementation progress only; no final-packet correctness, CLI self-law green proof, update_goal eligibility, registry/reviewer exposure, readiness, release, completion claim, install/cache parity, or `update_goal()` call.

## `update_goal()` Is Forbidden Until All Are True

- [ ] Status: in_progress
- [ ] Current source audit passes.
  - Evidence: Status: stale due to source change. Latest persisted source audit targets `sha256:ed82224ea1000ae486052a52787617086cdba1f4a88a2f8ad617429fb73f5d70`, status `fail`, and is stale relative to live implementation digest `sha256:895cdad77175a1041ce30e921327e10b74b2170441bbb961d6da01d77537cd08`. Claim impact: no current source-audit pass claim; readiness, release, completion, final-packet correctness, and `update_goal()` remain blocked.
- [ ] Current red fixture report passes with all fixtures failing for intended reasons.
  - Evidence: Status: stale due to source change. Latest persisted red fixture report targets `sha256:ed82224ea1000ae486052a52787617086cdba1f4a88a2f8ad617429fb73f5d70` and status `pass`, but it is stale relative to live implementation digest `sha256:895cdad77175a1041ce30e921327e10b74b2170441bbb961d6da01d77537cd08`. Claim impact: no current red-fixture pass claim, no source-audit pass, readiness, release, completion, final-packet correctness, or `update_goal()` claim.
- [ ] No standards law remains optional or unmechanized for material claims. Evidence: Status: stale due to source change. Latest standards/source-audit/standards-gardener evidence predates live implementation digest `sha256:895cdad77175a1041ce30e921327e10b74b2170441bbb961d6da01d77537cd08`. Claim impact: no current standards-complete claim.
- [ ] Foundational article law trace is complete and validator-enforced with no weak/deferral escape hatch.
  - Evidence: Status: stale due to source change. Latest foundational trace/source-obligation/source-audit evidence predates live implementation digest `sha256:895cdad77175a1041ce30e921327e10b74b2170441bbb961d6da01d77537cd08`. Claim impact: no current foundational-trace-complete claim.
- [ ] Plugin self-coverage is 100% with typed receipt and no uncovered records.
  - Evidence: Status: stale due to source change. The last exact coverage receipt predates live implementation digest `sha256:895cdad77175a1041ce30e921327e10b74b2170441bbb961d6da01d77537cd08`; package-check scheduler source changed after that proof. Claim impact: no current exact-coverage claim until the next phase-boundary coverage run.
...
- [ ] Line-cap adherence is enforced. Evidence: Status: implemented, pending validation. Current source-local line-cap observation is clean for package digest `sha256:895cdad77175a1041ce30e921327e10b74b2170441bbb961d6da01d77537cd08`: `find validator/src -name '*.rs' -exec wc -l {} + | awk '$2 != "total" && $1 > 250 { print }'` emitted no rows, `cargo fmt --check` exited `0`, and focused scheduler/package/red-fixture tests passed. This row remains unchecked because durable current source-audit/receipt-bound line-cap closure has not been rerun for this digest. Claim impact: source-local line-cap/source-topology evidence only; no readiness/release/completion/update_goal claim.
