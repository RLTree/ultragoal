...
- [ ] Agent-queryable observability status: in progress; command-row fitting validated current through `standards-gardener rebind`, and fitted rows reject unbound red/green/tamper labels. `source-obligations check` is the next incomplete command row.
