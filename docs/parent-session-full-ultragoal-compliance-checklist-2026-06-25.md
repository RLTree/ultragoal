...
- [ ] Agent-queryable observability status: in progress; research binding, inventory row-accounting enforcement, line-cap command/query fitting, namespace command/query fitting, and prior focused command fitting are validated current.
- [ ] Full local observability stack integration and non-opaque failure status: in progress; research binding, inventory/control-board enforcement, line-cap command/query fitting, and namespace command/query fitting are validated current while full fitting remains blocked.
