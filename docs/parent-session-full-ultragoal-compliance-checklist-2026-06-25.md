...
- [ ] Machine-readable operating-loop and signal inventory tracks whether observability is actually usable as the repair loop: current digest first, failing command capture, logs/metrics/traces query by run id, CLI explanation before manual artifact inspection, smallest repair, narrow rerun, before/after telemetry comparison, broad-audit gating, freshness, and the CLI-translated latency/traffic/error/saturation/freshness/correlation/redaction/boundedness signal model.
  - Status: in progress: same-candidate query/explain loop and row-contract accounting are validated current for this source-local slice.
...
- [ ] Run Gate 92 runtime detection, stack up, stack health, stack smoke, query logs, query metrics, query traces, explain current failure, focused observability tests, observability red/green/tamper fixtures, exact coverage, line-cap scan, source audit, red fixture report, package digest, git status, and checkpoint commit. Evidence: Status: in progress; row-contract inventory enforcement, control-board recomputation, focused observability tests, and narrow query/explain loop are validated current. Full Gate 92 fitting, source audit, red report, and live-stack closure remain pending.
...
- [ ] Agent-queryable observability status: in progress; research binding, row-contract inventory enforcement, and control-board recomputation are validated current; command rows without explicit pass/fail stdout proof are stale due to source change.
- [ ] Full local observability stack integration and non-opaque failure status: in progress; research binding, row-contract inventory enforcement, and control-board recomputation are validated current while full fitting and live-stack closure remain blocked.
