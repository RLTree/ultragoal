...
- [ ] Evidence path: Status: in progress. Current live implementation digest is `sha256:7bc8a9be3dbbe86affb7cad5c5eb05e0256dc8e51f2af3b056fe0b1f45f0f60d`. The scheduler/red-fixture isolated-temp parallel slice and exact coverage proof are validated current for source-local scope. Product/Fit/Journey, Rust/GC, standards-gardener, source audit, red report, final-packet, registry/app/install/cache, self-law, and update-goal receipts are stale or pending. Claim impact: source-local implementation progress only; no final-packet correctness, CLI self-law green proof, update_goal eligibility, registry/reviewer exposure, readiness, release, completion claim, install/cache parity, or `update_goal()` call.
...
- [ ] Line-cap adherence is enforced. Evidence: Status: validated current for source-local line-cap scan at package digest `sha256:7bc8a9be3dbbe86affb7cad5c5eb05e0256dc8e51f2af3b056fe0b1f45f0f60d`; durable source-audit/receipt-bound line-cap closure remains pending. Claim impact: source-local line-cap/source-topology evidence only; no readiness/release/completion/update_goal claim.
...
- [ ] Product Usage Fitness / routine CLI entrypoint is enforced. Evidence: Status: in progress. Product-usage slice is queued after the current scheduler/parallelization slice and before Phase 4 evidence rebinding. Claim impact: no routine-usability, product-readiness, readiness, release, completion, final-packet, or `update_goal()` claim until closed.
- [ ] CLI Help Discoverability / self-contained command guidance is enforced. Evidence: Status: in progress. Top-level and subcommand help must become self-contained enough for un-oriented agent/user discovery before routine-usability claims. Claim impact: no routine-usability claim until closed.
- [ ] Fit-repo and target-repo plugin-activated usage paths are executable and discoverable through the CLI. Evidence: Status: in progress. Plugin metadata and docs are not sufficient; executable CLI surfaces must expose the first-use and target-repo paths. Claim impact: no plugin-activated repo usability claim until closed.
- [ ] Receipt discipline and checklist progress-only policy are enforced. Evidence: Status: in progress. Checklist rows must remain concise statuses, and receipts may be minted or refreshed only at claim-boundary points. Claim impact: no claim can rely on checklist text or receipt churn.
- [ ] Observability-led narrow repair loop is enforced before broad audit reruns. Evidence: Status: in progress. Repeated broad audit/red-report loops remain invalid unless implementation or evidence semantics changed. Claim impact: no broad-boundary closure claim without narrow failure/query/explain/repair proof.
...
- [ ] Source/install/cache are same candidate and same digest. Evidence: Status: stale due to source change. Live source digest is `sha256:7bc8a9be3dbbe86affb7cad5c5eb05e0256dc8e51f2af3b056fe0b1f45f0f60d`; install/cache parity has not been refreshed and remains out of scope for this source-local slice. Claim impact: no disk package parity, no app-registry/reviewer exposure, no readiness, no release, no completion, and no `update_goal()` claim.
