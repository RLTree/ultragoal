...
- [ ] Machine-readable observability fitting inventory tracks every law-bearing CLI command, validator check family, receipt/proof path, fixture/report path, package surface, and plugin surface as `fitted`, `partially_fitted`, or `unfitted`, names the current owner surface and next unfitted surface for each row, includes a validator-recomputed `fitting_control_board`, and the validator fails every partial, unfitted, missing, stale, adjacent-surface-substituted, count-mismatched, pass-shaped-control-board, or row-shape-only fitting row.
  - Status: in progress; source-local inventory/control-board status is current for digest `sha256:5a8f24e0a00565a77f1c348af2afb56afeb81d843680e06a8350d8abbdf3ebd2`. `final-packet prove` remains partially fitted; next blocker is bounded low-card live metrics query proof.
...
- [ ] The current proof-graph failure is visible through stdout, source audit receipt, final-packet proof receipt, VictoriaLogs, VictoriaMetrics, VictoriaTraces, observe query commands, and `observe explain-failure --run-id`.
  ...
  - Next blocker: `final-packet prove` remains the first incomplete command row; next unfitted surface is bounded low-card live metrics query proof.
...
- [ ] Run Gate 92 runtime detection, stack up, stack health, stack smoke, query logs, query metrics, query traces, explain current failure, focused observability tests, observability red/green/tamper fixtures, exact coverage, line-cap scan, source audit, red fixture report, package digest, git status, and checkpoint commit. Evidence: Status: in progress; focused observability tests, build, fmt, line-cap scan, and package digest are current for the source-local inventory correction. Exact coverage, source audit, red report, and live-stack closure were not refreshed.
...
- [ ] Evidence path: Status: in progress. Current source-local subpath: nested law-receipt observability binding is validator-enforced, and `final-packet prove` is explicitly partial instead of fitted. Full Gate 92 remains blocked by bounded low-card metrics proof, exact coverage, full fitting, live-stack closure, source audit, red report, install/cache parity, final-packet correctness, and update_goal eligibility.
...
- [ ] Gate 92 evidence path: Status: in progress. Current digest is `sha256:5a8f24e0a00565a77f1c348af2afb56afeb81d843680e06a8350d8abbdf3ebd2`; `final-packet prove` is partially fitted with nested receipt binding enforced, and bounded low-card metrics proof remains the first command blocker. Claim impact: no final packet correctness, review readiness, package readiness, release readiness, completion, `update_goal()` eligibility, registry exposure, reviewer exposure, install/cache parity, transactional finalization, or full Gate 92 proof.
...
- [ ] Agent-queryable observability status: in progress; nested law-receipt observability binding is source-local validated, while bounded low-card metrics proof and remaining command/surface rows are still partial or unfitted.
- [ ] Full local observability stack integration and non-opaque failure status: in progress; exact coverage, full fitting, same-candidate command-query rebind, bounded metrics proof, and live-stack closure remain blocked.
