...
- [ ] Run Gate 92 runtime detection, stack up, stack health, stack smoke, query logs, query metrics, query traces, explain current failure, focused observability tests, observability red/green/tamper fixtures, exact coverage, line-cap scan, source audit, red fixture report, package digest, git status, and checkpoint commit. Evidence: Status: in progress. Exact coverage, line-cap, focused observability tests, logs query, and explain output are current; metrics/traces query fitting, broad Gate 92 inventory, source audit, and red report remain pending.
...
- [ ] Full local observability stack integration and non-opaque failure status: in progress; command fitting is validated current through foundational-trace check; coverage prove is the next command blocker.
