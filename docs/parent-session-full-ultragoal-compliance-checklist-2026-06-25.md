...
- [ ] Machine-readable observability fitting inventory tracks every law-bearing CLI command, validator check family, receipt/proof path, fixture/report path, package surface, and plugin surface as `fitted`, `partially_fitted`, or `unfitted`, names the current owner surface and next unfitted surface for each row, includes a validator-recomputed `fitting_control_board`, and the validator fails every partial, unfitted, missing, stale, adjacent-surface-substituted, count-mismatched, pass-shaped-control-board, or row-shape-only fitting row.
  - Status: in progress. Current first incomplete row is `final-packet prove`; `review-target build` and `archive build` are validated current for source-local command/query fitting. `final-packet prove` has validated current receipt-dereference child spans, pass/fail stdout contract, source-local explain output, current same-candidate logs/metrics/traces query proof, and production control-board first-incomplete validation, and remains partial on full command-inventory closure. Earlier fitted command rows remain implemented/fitted, but their same-candidate query receipts are stale due to source change and need boundary rebind. Other command, surface, loop, signal, validator-check, receipt, fixture, setup, long-running, external/live, and claim-guard rows remain partial or unfitted.
...
- [ ] Run coverage command proving 100%. Evidence: Status: stale due to source change. Claim impact: no current exact-coverage claim; no readiness, release, completion, final-packet, registry/reviewer, or `update_goal()` claim.
...
- [ ] Run Gate 92 runtime detection, stack up, stack health, stack smoke, query logs, query metrics, query traces, explain current failure, focused observability tests, observability red/green/tamper fixtures, exact coverage, line-cap scan, source audit, red fixture report, package digest, git status, and checkpoint commit. Evidence: Status: in progress. Full Gate 92 fitting, source audit, red report, and live-stack closure remain pending.
- [ ] Run source/install/cache digest comparison. Evidence: Status: stale due to source change. Live source digest is `sha256:faf8a298726f7228f58c34bcb6f36065376374f020b5d832bc811c79fba65301`; install/cache parity has not been refreshed for this source-local checkpoint. Claim impact: no disk source/install/cache parity, no app-registry/reviewer exposure, no final-packet correctness, no readiness, no release, no completion, and no `update_goal()` claim.
...
- [ ] Plugin self-coverage is 100% with typed receipt and no uncovered records.
  - Evidence: Status: validated current. Claim impact: source-local coverage proof only; no readiness/release/completion/final-packet/registry/update_goal claim.
...
- [ ] Agent-queryable observability status: in progress; research binding, row-contract inventory enforcement, control-board recomputation, `review-target build`, `archive build`, and `final-packet prove` receipt-dereference child-span, pass/fail stdout, production control-board guard, explain output, and current same-candidate query fitting are validated current. Earlier fitted command query receipts are stale due to source change and need boundary rebind; remaining command/surface rows still partial or unfitted.
