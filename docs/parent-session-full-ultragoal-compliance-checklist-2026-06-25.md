...
- [ ] Gate 92 evidence path: Status: in progress. Command-row fitting is validated current through `coverage prove`; the first incomplete command row is `line-cap check`. Coverage command observability is fitted, but exact coverage still fails source-local on uncovered code and stale coverage receipt digests; source audit and red fixture report remain stale/failing, and full Gate 92 remains blocked. Claim impact: no final packet correctness, review readiness, package readiness, release readiness, completion, `update_goal()` eligibility, registry exposure, reviewer exposure, install/cache parity, transactional finalization, or full Gate 92 proof.
